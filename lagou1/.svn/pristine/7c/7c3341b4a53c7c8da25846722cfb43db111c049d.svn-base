...
public class AdminsDAO {
	 ...
	 * @param adminsId
	 */
	public void delete(int adminsId) {
		log.debug("deleting Admins instance");
		
		try {
			String hql="delete from com.logou.domain.Admins where adminId = ?";
			Query query=getCurrentSession().createQuery(hql);
			query.setParameter(0, adminsId);
	...
	public Admins findByName(String adminName) {
		try {
			Criteria criteria = getCurrentSession().createCriteria(Admins.class);
			Criterion criterion=Restrictions.eq("adminName", adminName);
			criteria.add(criterion);
			return (Admins) criteria.uniqueResult();
	...
	public List<Admins> findByExample(Admins instance) {
		...
		try {
			List<Admins> results = (List<Admins>) getCurrentSession()
					.createCriteria("com.lagou.domain.Admins")

	...
	public List<Funs> findByRolesId(java.lang.Integer id) {
		...
		try {
			Roles instance = (Roles) getCurrentSession().get(
					"com.lagou.domain.Roles", id);
			List<Funs> funsList=instance.getFunses();
			for (Funs funs : funsList) {
				Hibernate.initialize(funs.getFunses());
			}
			return funsList;
	...
	public List<Roles> findByExample(Roles instance) {
		...
		try {
			List<Roles> results = (List<Roles>) getCurrentSession()
					.createCriteria("com.lagou.domain.Roles")

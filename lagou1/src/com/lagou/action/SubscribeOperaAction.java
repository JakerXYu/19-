...
public class SubscribeOperaAction extends ActionSupport {
	...
	@Action(value="delect" , results={
			@Result(name="ok" , location="/jobSeeker/mySubscribedJob.jsp"),
			@Result(name="on" , location ="/jobSeeker/mySubscribedJob.jsp")
	})
	public String delect(){
		int subscribeid = Integer.parseInt(subscribeId);
		Subscribe subscribe = subscribeBIZ.findById(subscribeid);
		if(subscribeBIZ.delect(subscribe)){
			return "ok";
